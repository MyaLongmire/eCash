...
pub_key = (17344639424944367590650082428629712733401683850414502668706531968772859428351L, 27450403844741214695492334021007303484379951889068003858532711256412335333083L)
priv_key = (17138083845453581670252493603505692112752830897246138493778413083207291679L, 27450403844741214695492334021007303484379951889068003858532711256412335333083L)
...
#show signed message
def show(message, status, pub_key):
    hidden_msg = int(message)
    final = (hidden_msg * calc_inv(pub_key[1], status)) % pub_key[1]
    return str(final)

#bank verifying for merchant
def verify(message, pub_key):
    return str(pow(int(message), * pub_key) % pub_key[1])
...
def c_order():
    ...
    for i in range(0,n):
        b_order[i], factor[i] = hide(order[i], pub_key)
...
def m_verify():
    ...
    o_verify = verify(o_recieved, pub_key)
...
#sign hidden msg
def b_sig(message, priv_key):
    return str(pow(int(message), *priv_key))
...
def b_verify():
    ...
    for i in r:
        m_signed[i] = b_sig(b_order[i], priv_key)
        m_show[i] = show(m_signed[i], factor[i], pub_key)
        o_verified[i] = verify(m_show[i], pub_key)
    ...
    for j in range(0,10):
        if j not in r:
            m_signed = b_sig(b_order[j], priv_key)
...
def c_show(x):
    ...
    show_order = show(sb_order, factor[x], pub_key)

    with open('shown and signed orders', 'wb') as f:
        cPickle.dump(show_order, f)

#hide msg
def hide(message, pub_key):
    factor = f_hide(pub_key[1])
    msg = int(message)
    hide_msg = (pow(factor, *pub_key)*msg) % pub_key[1]
